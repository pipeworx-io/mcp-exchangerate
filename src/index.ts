...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'ExchangeRate');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'get_pair',
    description: 'Get the current exchange rate between two specific currencies (e.g., USD to EUR). Returns the conversion rate. Use for single currency pair lookups.',
    inputSchema: {
      type: 'object',
      properties: {
        from: {
...
function reqStr(args: Record<string, unknown>, key: string): string {
  ...
  if (typeof v !== 'string' || !v.trim()) {
    throw new Error(`Required argument "${key}" is missing or empty. Pass a 3-letter currency code like "USD", "EUR", or "JPY".`);
  }
  return v;
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'get_rates':
      return getRates(reqStr(args, 'base_currency'));
    case 'get_pair':
      return getPair(reqStr(args, 'from'), reqStr(args, 'to'));
    default:
...
async function fetchLatest(base: string): Promise<ErApiResponse> {
  const res = await pwFetch(`${BASE_URL}/latest/${encodeURIComponent(base.toUpperCase())}`);
  if (!res.ok) throw await httpError(res, 'open.er-api.com error');
