...
`tools/list` at `https://gateway.pipeworx.io/exchangerate/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_rates \
  -H 'Content-Type: application/json' \
  -d '{"base_currency":"USD"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_rates`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "exchangerate": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-exchangerate"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-exchangerate
```
